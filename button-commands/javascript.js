...
const { ansRow, javascriptContRow } = require('../assets/action-rows');

const saName = 'javascript';
		...
		buttonPressMsg.awaitMessageComponent({ componentType: ComponentType.Button, time: secPerQuestion, max: 1 })
			.then(i => {
				...
				if (i.customId === correctResponse) {
					...
					i.update(
						{ content: `✅ Answer ${i.customId.toUpperCase()} is correct!.\nI'll ask you again on ${newAtmp.next_up}`, embeds: [embed], components: [javascriptContRow] },
					);
				} else {
					...
					i.update(
						{ content: `Sorry, ${i.customId.toUpperCase()} is not right. I'll ask you again later.`, embeds: [embed], components: [javascriptContRow] },
					);
				}
			}, (reason => {
				...
				interaction.editReply(
					{ content: 'Timed out... you really took your time with this one! Attempt cleared from database; this won\'t affect your stats.', embeds: [], components: [javascriptContRow] },
				);
			}))
			.catch(err => {
				console.log(`🚫⌛ No response collected!\nError: ${err}.`);
				...
				interaction.editReply(
					{ content: `Error collecting response: ${err}.\n\nYour grade was not affected. If this continues, run /skill-assessment again to resume.`, embeds: [], components: [javascriptContRow] },
				);
			});
	},
};
...
// path is Node's native path utility module. It helps construct paths to access files and directories. No need to include '/' just the path names. It automatically detects the operating system and uses the appropriate joiners.
...
// build an events handler very similar to commands handler below
...
// Collection is an extenstion of the JS Native 'Map' class with additional utility methods. This is used throughout discord.js rather than Arrays for anything that has an ID, for significantly improved performance and ease-of-use. Constructor: new Collection(entries);
...
// iterate through each 'file' of 'commandFiles' and construct a full path, called 'filePath', for each command.js file. Set the path = 'command'
for (const file of commandFiles) {
	const filePath = path.join(commandsPath, file);
	const command = require(filePath);
	
	// Then, add each file to the 'client.commands' Collection()
	// The 'key' @param is a string of each command name (from 'SlashCommandBuilder().setName' which has been renamed to 'data' + .name)
	// The 'value' @param is the exported module (everything in the 'module' from each command.js file)
	client.commands.set(command.data.name, command);
	// so now the Collection is filled with { 'string' => definedInteractionFunction }
}
...
client.on('interactionCreate', async interaction => {
	...
	// grab the interaction code from the Collection()
	const command = client.commands.get(interaction.commandName);

	// exit out if the command doesn't exist ('undefined')
	if (!command) return;

	// execute the code from 'interaction'
	try {
